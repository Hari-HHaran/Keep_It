      ...
      <div className="space-y-2">
        ...
        <div className="rounded-2xl bg-[#FFFDF8] border border-[#E0D4BF] overflow-hidden shadow-sm transition">
          <button
            ...
            className="w-full p-4 flex items-center justify-between text-left hover:bg-[#F5F1E7] transition"
          >
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="text-sm font-bold text-[#1B1815]">
                  {bankAccounts.length} bank accounts linked
                </div>
                <div className="text-xs text-[#8A8075]">
                  DBS, OCBC • synced 4 min ago
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="text-[#8A8075] p-1 rounded-full bg-[#F5F1E7]">
                {isBankExpanded ? (
          ...
          {isBankExpanded && (
            <div className="px-4 pb-4 pt-1 border-t border-[#EDE4D6] space-y-2 bg-[#FBF6EC] animate-slideDown">
              ...
              {bankAccounts.map((account) => (
                <div
                  key={account.id}
                  className="p-3 rounded-xl bg-[#FFFDF8] border border-[#E0D4BF] flex items-center justify-between shadow-xs"
                >
                  <div className="flex items-center space-x-2.5">
                    <div className="h-7 w-7 rounded-lg bg-[#DDE8E1] flex items-center justify-center text-[10px] font-mono-custom font-bold text-[#0F4635]">
                      {account.bankName.slice(0, 3)}
                    </div>
                    <div>
                      ...
                      <div className="text-[10px] text-[#8A8075]">
                        {account.accountNumber} • {account.lastSynced}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-[9px] text-[#0F4635] font-semibold">
                      ✓ Connected
                    </div>
                  </div>
                </div>
          ...
          {isWalletExpanded && (
            <div className="px-4 pb-4 pt-1 border-t border-[#EDE4D6] space-y-2 bg-[#FBF6EC] animate-slideDown">
              {walletAccounts.map((w) => (
                <div
                  key={w.id}
                  className="p-3 rounded-xl bg-[#FFFDF8] border border-[#E0D4BF] flex items-center justify-between text-xs"
                >
                  <div>
                    <div className="font-bold text-[#1B1815]">{w.bankName}</div>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-[10px] text-[#9A7420] font-medium">
                      Instant PayNow
                    </div>
                  </div>
                </div>
